...
// io_context：平台事件循环（Linux epoll）+ 执行器。形态取自 Networking TS 的
// io_context：run() 在调用线程上运行事件循环直到没有未完成的工作或 stop()；多个线程
// 可以同时 run() 同一个上下文；executor_type 满足 Executor 概念。
//
// 工作计数：run_async 与 run 在链存续期间持有一份工作；反应器为每个排队中的 I/O 操作
// 与定时器持有一份。计数归零且队列为空时 run() 返回。
//
...
struct io_context final : execution_context {
